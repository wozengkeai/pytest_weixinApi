...
class ManageMember(BaseAPI):
    ...
    #获取部门成员
    def get_member_list(self,departmentid):
        with allure.step("获取部门成员"):
            url = self.yaml_data["contacts"]["member"]["get_dep_mem_list"]
            params = {
                "access_token": self.access_token,
            ...
            logging.debug("url:" + str(url))
            logging.debug("params:" + str(params))
            with allure.step("发出获取部门成员的请求"):
                self.send_get(url, params=params)

    #获取部门成员详情
    def get_member_detail(self,departmentid):
        with allure.step("获取部门成员详情"):
            url = self.yaml_data["contacts"]["member"]["get_dep_mem_detail"]
            params = {
                "access_token": self.access_token,
                "department_id": departmentid
            }
            logging.debug("url:" + str(url))
            logging.debug("params:" + str(params))
            with allure.step("发出获取部门成员详情的请求"):
                self.send_get(url, params=params)
